...
**Prompt path (bridge):** `CURSOR_API_KEY` (env, or gitignored local config — [API_KEY.md](API_KEY.md)) + `--cwd` workspace. Local `cursor-sdk` via **async** `AsyncClient.launch_bridge` (Windows: `node.exe` + bridge `.js`, not `.cmd`) + create/send (or resume when `agentId` set). Ping/pong unchanged.
